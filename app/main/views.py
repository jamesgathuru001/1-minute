...
def index():
    ...
    View root page function that returns the index page and its data
    '''
    general = Pitch.query.filter_by(
        category="general").order_by(Pitch.posted.desc()).all()
    project = Pitch.query.filter_by(
        category="project").order_by(Pitch.posted.desc()).all()
    advertisement = Pitch.query.filter_by(
        category="advertisement").order_by(Pitch.posted.desc()).all()
    sale = Pitch.query.filter_by(category="sale").order_by(
        Pitch.posted.desc()).all()
...
def index2():
    ...
    '''
    advertisement = Pitch.query.filter_by(
        category="advertisement").order_by(Pitch.date.desc()).all()
    project = Pitch.query.filter_by(
        category="project").order_by(Pitch.date.desc()).all()
    general = Pitch.query.filter_by(
        category="general").order_by(Pitch.date.desc()).all()
    sale = Pitch.query.filter_by(
        category="sale").order_by(Pitch.date.desc()).all()
...
def comment(pitch_id):
    ...
    if comment_form.validate_on_submit():
        ...
        new_comment = Comment(comment=comment_body,
                              pitch_id=pitch_id, user=current_user)
...
def advertisement():
    ...
    if form.validate_on_submit():
        ...
        new_advertisement = Advertisement(
            post=post, user=current_user, body=body)
